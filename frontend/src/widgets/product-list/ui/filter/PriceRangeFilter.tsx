...
const PriceRangeFilter = ({
  ...
  // Храним предыдущие значения для синхронизации
  const [prevMin, setPrevMin] = useState<number | null>(priceMin);
  const [prevMax, setPrevMax] = useState<number | null>(priceMax);

  const [localRange, setLocalRange] = useState<[number, number]>([currentMin, currentMax]);
  
  const [inputMin, setInputMin] = useState(
    currentMin === min ? "" : formatNumber(currentMin),
  );
  const [inputMax, setInputMax] = useState(
    currentMax === max ? "" : formatNumber(currentMax),
  );

  // Синхронизация стейта при внешнем сбросе фильтров
  if (priceMin !== prevMin || priceMax !== prevMax) {
    setPrevMin(priceMin);
    setPrevMax(priceMax);
    
    setLocalRange([currentMin, currentMax]);
    setInputMin(currentMin === min ? "" : formatNumber(currentMin));
    setInputMax(currentMax === max ? "" : formatNumber(currentMax));
  }
  ...
  const commitInput = (type: "min" | "max") => {
    ...
    onChange(finalRange[0], finalRange[1]);
  };
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1">
          <input
            ...
            className="h-10 w-full px-3 text-[16px] font-bold font-roboto_condensed border border-slate-950 rounded-sm focus:outline-none focus:border-slate-500"
          />
        </div>
