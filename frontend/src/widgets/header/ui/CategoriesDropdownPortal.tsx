...
const CategoriesDropdownPortal = ({
  ...
  return createPortal(
    <>
      ...
      >
        <div className="grid grid-cols-6 gap-5 gap-y-10 container p-5">
          {categoriesData.map((cat) => (
            <div key={cat.href}>
              <Link
                href={cat.href}
                className="block text-sm font-medium mb-4 hover:underline leading-4"
                onClick={onClose}
              >
                {cat.title}
              </Link>

              <div className="flex flex-col gap-3 text-xs leading-3.5">
