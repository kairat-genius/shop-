...
const RelatedBrandsThemes = ({ className }: RelatedBrandsThemesProps) => {
  return (
    <section className={cn("px-[3.733vw] my-[3.733vw]", className)}>
      <Accordion
        chevronClassName="w-4 h-4"
        className="gap-[1.067vw] mb-0"
        title={
          <h2 className="text-[3.733vw] font-medium">
            СВЯЗАННЫЙ БРЕНД ИЛИ ТЕМЫ
          </h2>
        }
      >
        <div className="mt-[3.733vw]">
          <div className="grid grid-cols-2 gap-y-[2.667vw] gap-x-[5.333vw]">
            {brands.map((item) => (
              <Link
                key={item.href}
          ...
          <div className="my-[2.667vw] h-px bg-slate-100" />
          <div className="grid grid-cols-2 gap-y-[2.667vw] gap-x-[5.333vw]">
            {themes.map((item) => (
              <Link
                key={item.href}
